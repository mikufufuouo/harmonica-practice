...
const SEMITONES: Record<Key, number> = { C: 0, G: -5, A: -3, D: 2, F: 5, Bb: -2 };
...
export function noteFor(key: Key, hole: number, breath: Breath): string {
  ...
  return key === 'Bb' && note === 'A#' ? `Bb${octave}` : note === 'A#' ? `A#${octave}` : `${note}${octave}`;
}

export function isConsecutiveHoles(holes: readonly number[]): boolean {
  if (holes.length === 0 || holes.some((hole) => !Number.isInteger(hole) || hole < 1 || hole > 10)) return false;
  const sorted = [...new Set(holes)].sort((a, b) => a - b);
  return sorted.length === holes.length && sorted.every((hole, index) => index === 0 || hole === sorted[index - 1] + 1);
}
