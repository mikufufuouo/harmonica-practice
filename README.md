...
页面提供纯文本 BD 预览，以及“图片识别与本地谱库（实验）”面板。后者把单张图片 Blob、OCR 原文和校对草稿保存在浏览器 IndexedDB；Tesseract.js Worker 首次识别会按需下载语言模型，图片不会上传。OCR 结果必须逐行分类、核对，并明确确认已对照原图检查无漏行，才能进入本地谱库。谱库可恢复草稿、查看原图和指法、导出/导入经过运行时校验的 JSON、删除条目。JSON 备份不含原图。

请先在相册裁剪清晰短图；首版没有应用内裁剪、自动长图切块或逐音符图片定位。歌曲和草稿仅保存在当前浏览器，清除站点数据会删除它们；歌曲可主动导出备份，原图请另行保留。离线外壳和本地谱库不依赖 OCR 模型下载，OCR 的完全离线运行尚未验收。
